...
#include <stdlib.h>
#define UP "ABCDEFGHIJKLMNOPQRSTVUWXYZ"
#define LOW "abcdefghijklmnopqrstvuwxyz"
...
#elif __linux__
    #define EMAX 131
    #define S21_ERRORS { "Operation not permitted", \
        "No such file or directory", \
        "No such process", \
        "Interrupted system call", \
        "Input/output error", \
        ...
        "Too many referenc cannot splice", \
...
int s21_sprintf(char *str, const char *format, ...);                // printf

void *s21_to_upper(const char *str);                                    //  lower into upper
void *s21_to_lower(const char *str);                                    //  upper into lower
